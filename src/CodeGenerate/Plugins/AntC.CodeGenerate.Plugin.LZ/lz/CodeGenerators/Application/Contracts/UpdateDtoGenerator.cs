...
{
    public class UpdateDtoGenerator : BaseTableCodeGenerator
    {
        public override GeneratorInfo GeneratorInfo => new GeneratorInfo()
        {
            Name = "Lz.Dto.Update",
            Desc = "此模板生成基于Abp的 更新 数据传输对象"
        };
        ...
        protected override GeneratorConfig GetDefaultConfig(TableCodeGenerateContext context)
        {
            return new GeneratorConfig()
            {
                FileRelativePath = Path.Combine("Application.Contracts",
                    ...
                    $"Update{context.ClassInfo.ClassName}Dto.cs")
            };
        }
        ...
        public override void ExecutingCodeGenerate(TableCodeGenerateContext context)
        {
            ...
            context.AppendLine($"    /// 更新 {context.ClassInfo.Annotation} 数据传输对象");
            context.AppendLine($"    /// </summary>");
            context.AppendLine($"    public class Update{context.ClassInfo.ClassName}Dto : CreateUpdate{context.ClassInfo.ClassName}Dto,IHasConcurrencyStamp");
