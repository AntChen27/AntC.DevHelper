...
using AntC.CodeGenerate.Plugin.LZ.lz.CodeGenerators.Application.Contracts;
...
namespace AntC.CodeGenerate.Plugin.LZ.lz.CodeGenerators.Application
{
    public class AutoMapperProfileGenerator : BaseTableCodeGenerator
    {
        ...
        {
            Name = "Lz.Application.AutoMapper",
        ...
        public override void ExecutingCodeGenerate(TableCodeGenerateContext context)
        {
            ...
            if (context.CodeGeneratorContainer.ContainsCodeGenerator(typeof(CreateDtoGenerator)))
            {
                context.AppendLine(
                    $"            CreateMap<Create{context.ClassInfo.ClassName}Dto, {context.ClassInfo.ClassName}>();");
            }

            if (context.CodeGeneratorContainer.ContainsCodeGenerator(typeof(UpdateDtoGenerator)))
            {
                context.AppendLine(
                    $"            CreateMap<Update{context.ClassInfo.ClassName}Dto, {context.ClassInfo.ClassName}>();");
            }
